...
Claude Code має функцію **auto-compact**, яка автоматично запускається, коли контекстний простір стає заповненим (близько 80% від ліміту). Ця функція допомагає управляти пам'яттю, але може призвести до часткової втрати контексту поточної сесії. Хоча на практиці досить розумно працює. Також можна вручну запустити команду `/compact` в потрібний момент для оптимізації використання контексту. Цей момент зручніше ніж у Augment, бо там просто повідомлення, що робота з довгим обговоренням менш ефективна, а Claude просто автоматично покращує це сам.